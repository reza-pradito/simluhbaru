...
class PenyuluhPNSModel extends Model
{
    ...
    public function getPenyuluhPNSTotal($kode)
    {
        //d($kode_kab);
        $db = Database::connect();
        ...
        where a.tempat_tugas='$kode' and status !='1' and status !='2' and status !='3'");
            $row   = $query->getRow();

            $query   = $db->query("select a.nip_lama, a.nip, a.nama, a.gelar_dpn, a.gelar_blk, a.tgl_update, k.nama_bpp,
                                case a.status
                                when '0' then 'Aktif'
                                ...
                                left join tbldaerah j on a.kecamatan_tugas=j.id_daerah
                                where a.tempat_tugas='$kode' and status !='1' and status !='2' and status !='3' order by nama");
            ...
            $query = $db->query("select count(a.id) as jum, nama_dati2 as nama_kab from tbldasar a left join tbldati2 b on b.id_dati2=a.satminkal where satminkal='$kode' and status !='1' and status !='2' and status !='3' and status !='7'");
            $row   = $query->getRow();
            ...
            $query   = $db->query("select a.noktp, a.nip, a.nama, a.gelar_dpn, a.gelar_blk, a.tgl_update, d.nm_desa,
                                    ...
                                    left join tbldaerah s on a.kecamatan_tugas9=s.id_daerah
                                    left join tbldaerah t on a.kecamatan_tugas10=t.id_daerah
                                    where a.satminkal='" . $kode . "' and status !='1' and status !='2' and status !='3' and status !='7' order by nama");
            $results = $query->getResultArray();
